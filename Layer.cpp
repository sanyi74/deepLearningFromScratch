...
Layer* Layer::initW(std::string initMethod) {
	if (initMethod == "random") {
		for (size_t i = 0; i < width * prevWidth; i++) W[i] = 2.0 * std::ceil(1000.0*randUniform()) / 1000.0 - 1.0;
	}
	else if (initMethod == "positiveRandom") {
		for (size_t i = 0; i < width * prevWidth; i++) W[i] = std::ceil(1000.0*randUniform()) / 1000.0;
	}
	else if (initMethod == "zero") {
		memset(W, 0, width * prevWidth * sizeof(double));
	}
	return this;
}
